...
-- Tìm ProximityPrompt "Enter the rift" trên portal
...
-- Kiểm tra đã vào rift chưa qua DungeonDataSetEnteredSync
...
local function getPortalForRift(arenaId)
    ...
    -- Fallback: scan toàn bộ workspace tìm ProximityPrompt "Enter the rift" + arenaId
    for _,island in ipairs(area:GetChildren())do
        local islandArea=island:FindFirstChild("Area");if not islandArea then continue end
        local dungeon=islandArea:FindFirstChild("Dungeon");if not dungeon then continue end
        for _,dm in ipairs(dungeon:GetChildren())do
            for _,child in ipairs(dm:GetChildren())do
                -- Tìm prompt trong child
...
-- Chọn Rift tốt nhất theo priority
...
-- Teleport sát portal (đủ gần để ProximityPrompt kích hoạt)
...
local function joinRift(arenaId)
    ...
    task.wait(1.5) -- chờ server nhận vị trí

    -- BƯỚC 2: Fire ProximityPrompt (bấm E mở UI rift)
    if portal then
...
-- Leave Rift khi cần thoát thủ công
...
local function riftLoop()
    ...
    while riftRunning do
        ...
        -- Join + Start Rift (ProximityPrompt → CreateTeam → JoinTeam → Start)
