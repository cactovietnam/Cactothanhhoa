...
-- DEBUG SCANNER
mkSec("rift","🔬  Debug Scanner",Color3.fromRGB(60,120,220))
local dbStat=mkStat("rift","📋  Chưa scan...")
local dbStat2=mkStat("rift","")

-- Nút 1: Dump tất cả key trong DS2 ra console
mkBtn("rift","Dump DS Keys","In key DS2 ra Roblox console",Color3.fromRGB(40,80,180),function()
    task.spawn(function()
        dbStat.Text="🔍  Đang scan DS2..."
        if not ensureDS() then dbStat.Text="❌  Load DS2 thất bại!";return end
        local keys={}
        for k,v in pairs(DS2)do
            local t=type(v)
            table.insert(keys,"["..t.."] "..tostring(k))
        end
        table.sort(keys)
        print("=== DS2 KEYS ("..#keys..") ===")
        for _,s in ipairs(keys)do print(s)end
        print("=========================")
        dbStat.Text="✅  Đã in "..#keys.." keys → xem Roblox Console"
    end)
end)

-- Nút 2: Tìm RemoteEvent/Function trong RS liên quan dungeon/arena/rift
mkBtn("rift","Scan Remotes","Tìm remote dungeon trong RS",Color3.fromRGB(40,140,80),function()
    task.spawn(function()
        dbStat2.Text="🔍  Đang scan RS..."
        local found={}
        local function scanRS(obj,path)
            for _,c in ipairs(obj:GetDescendants())do
                if c:IsA("RemoteEvent") or c:IsA("RemoteFunction") then
                    local n=c.Name:lower()
                    if n:find("dung") or n:find("arena") or n:find("rift") or n:find("boss") or n:find("join") or n:find("start") or n:find("enter") then
                        local full=c:GetFullName()
                        table.insert(found,c.ClassName.." | "..full)
                    end
                end
            end
        end
        scanRS(RS,"RS")
        print("=== DUNGEON REMOTES ("..#found..") ===")
        for _,s in ipairs(found)do print(s)end
        print("====================================")
        dbStat2.Text="✅  "..#found.." remotes → xem Roblox Console"
    end)
end)

-- Nút 3: Test TP + ProximityPrompt arenaId=31
mkBtn("rift","Test TP+Prompt 31","Chỉ test teleport & prompt",Color3.fromRGB(100,60,180),function()
    task.spawn(function()
        dbStat.Text="🚶  TP portal 31..."
        local ok,portal=tpToPortal(31)
        if not ok then dbStat.Text="❌  Không thấy portal 31!";return end
        dbStat.Text="✅  Portal ok. Tìm prompt..."
        task.wait(0.5)
        local pr=findRiftPrompt(portal)
        if not pr then dbStat.Text="⚠️  Không thấy ProximityPrompt!";return end
        dbStat.Text="✅  Prompt: \""..pr.ActionText.."\" → Fire!"
        pcall(function()fireproximityprompt(pr)end)
        task.wait(1)
        dbStat.Text="🔑  Đã fire prompt. UI rift có mở không?"
    end)
end)
