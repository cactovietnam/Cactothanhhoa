...
-- Tìm tất cả portal trong map
local function findAllPortals()
    local result={}
    local area=workspace:FindFirstChild("Area");if not area then return result end
    for _,island in ipairs(area:GetChildren())do
        ...
        for _,dm in ipairs(dungeon:GetChildren())do
            for _,child in ipairs(dm:GetChildren())do
                if child.Name:lower():find("portal")then
                    -- Lấy vị trí
                    local pos=nil
                    ...
                    if pos then table.insert(result,{model=child,pos=pos,portalName=child.Name})end
                end
            end
        end
    end
...
local function getBestRift()
    ...
    elseif PRIOR=="high" then table.sort(candidates,function(a,b)return a.rift.diff>b.rift.diff end)
        ...
        local i=math.random(1,#candidates);return candidates[i].rift,candidates[i].pos
    end
...
local function tpToPortal(arenaId)
    local _,pos=getPortalForRift(arenaId)
...
local function joinRift(arenaId)
    ...
    -- Teleport đến đúng portal
    rStat.Text="🚶  Teleport portal..."
    if not tpToPortal(arenaId) then rStat.Text="❌  Không tìm thấy portal!";return false end
...
local function startRift(arenaId)
    ...
    local ok3,res=pcall(function()return VU.DoRequest(DS.ClientStartDungeon,arenaId)end)
    return ok3 and res
end
...
local function riftLoop()
    ...
    while riftRunning do
        ...
        if not joined then
            rStat.Text="❌  Join thất bại! Thử lại..."
