...
local function loadS()
    if not readfile then return DEF end
...
local function saveS(s)if writefile then pcall(writefile,SF,HS:JSONEncode(s))end end
local C=loadS()
...
-- HOP LOGIC (fixed: tự động thử server tiếp theo khi gặp lỗi 772/773)
local hop2=false
local function doHop()
    if hop2 then return end
    hop2=true
    hStat.Text="⏳  Đang tìm server..."
    local d=getSvr()
    if not d or not d.data or #d.data==0 then hStat.Text="❌  Lỗi API!";hop2=false;return end
    addRec(game.JobId)
    -- Lọc server hợp lệ + buffer 2 slot tránh lỗi 772
    local servers={}
    for _,s in ipairs(d.data)do
        if s.id~=game.JobId and not isRec(s.id) then
            local maxP=s.maxPlayers or 20
            if s.playing<=maxP-2 then table.insert(servers,s)end
        end
    end
    ...
    local function tryNext()
        idx=idx+1
        if idx>#servers then
            hStat.Text="⚠️  Tất cả server đều lỗi!"
            if conn then conn:Disconnect();conn=nil end
            hop2=false;done=true;return
        end
        local s=servers[idx]
        hStat.Text=string.format("🔄  Thử %d/%d (%d người)...",idx,#servers,s.playing)
        addRec(s.id)
        pcall(function()TS:TeleportToPlaceInstance(placeId,s.id,player)end)
    end
    -- Bắt lỗi teleport (772=full, 773=restricted, ...)
    conn=TS.TeleportInitFailed:Connect(function(plr,result,msg)
        if plr~=player then return end
        hStat.Text=string.format("❌  Lỗi %s – thử tiếp...",tostring(result))
    ...
    task.delay(60,function()
        if not done then
            if conn then conn:Disconnect();conn=nil end
            hop2=false;hStat.Text="⏹  Timeout, thử lại sau"
        end
    end)
end
...
local function findRiftPrompt(portalModel)
    for _,desc in ipairs(portalModel:GetDescendants())do
        if desc:IsA("ProximityPrompt") and desc.ActionText:lower():find("enter")then return desc end
    end
    return nil
end
    ...
    if CD2 then
        ...
        if cfg and cfg.EnterModel then
            for _,island in ipairs(area:GetChildren())do
                local islandArea=island:FindFirstChild("Area");if not islandArea then continue end
                local dungeon=islandArea:FindFirstChild("Dungeon");if not dungeon then continue end
                for _,dm in ipairs(dungeon:GetChildren())do
                    ...
                    if portal then
                        ...
                        if pos then return portal,pos end
                    end
                end
            end
        end
        ...
        for _,dm in ipairs(dungeon:GetChildren())do
            for _,child in ipairs(dm:GetChildren())do
                for _,desc in ipairs(child:GetDescendants())do
                    if desc:IsA("ProximityPrompt") and desc.ActionText:lower():find("enter")then
                        local pos=nil
                        if child:IsA("BasePart")then pos=child.Position
                        elseif child.PrimaryPart then pos=child.PrimaryPart.Position
                        else for _,p in ipairs(child:GetDescendants())do if p:IsA("BasePart")then pos=p.Position;break end end end
...
local function joinRift(arenaId)
    if not ensureDS() then rStat.Text="❌  Không load được DungeonSystem!";return false end
    ...
    if portal then
        ...
        if prompt then
            rStat.Text="🔑  Mở UI Rift (ProximityPrompt)..."
            if fireproximityprompt then pcall(function()fireproximityprompt(prompt)end)end
            task.wait(0.8)
        else
            rStat.Text="⚠️  Không thấy prompt, thử trực tiếp..."
        end
    end
    rStat.Text="👥  Tạo team..."
    pcall(function()VU2.DoRequest(DS2.ClientCreateTeam,arenaId)end)
    task.wait(0.8)
    rStat.Text="🤝  Join team..."
    pcall(function()VU2.DoRequest(DS2.ClientJoinTeam,arenaId)end)
    task.wait(0.8)
    rStat.Text="▶  Start Dungeon..."
    pcall(function()VU2.DoRequest(DS2.ClientStartDungeon,arenaId)end)
    task.wait(1.5)
    if checkEntered(arenaId) then rStat.Text="✅  Đã vào Rift!";return true end
    rStat.Text="🔄  Retry Start..."
    pcall(function()VU2.DoRequest(DS2.ClientStartDungeon,arenaId)end)
    task.wait(1.5)
...
local function hopForNewRift()
    ...
    if bestRift then rStat.Text="🔄  Tìm thấy Rift mới: "..bestRift.name;return true end
...
print("✅ NGUYEN CUTO v8.0 - Ready!")tLoop)end
end)
