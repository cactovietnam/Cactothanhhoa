...
local function doRift()
    ...
    if not ok or not ok2 then rStat.Text="❌  Lỗi!";return end
    rStat.Text="🌀  Tạo team..."
    local ok3=pcall(function()VU.DoRequest(DS.ClientCreateTeam,RI)end)
    task.wait(1)
    rStat.Text="▶  Start Rift..."
    pcall(function()VU.DoRequest(DS.ClientStartDungeon,RI)end)
    rStat.Text="✅  Started: "..RN
end
rBtn.MouseButton1Click:Connect(function()pcall(doRift)end)
task.spawn(function()
    task.wait(5);if FL then applyFixLag()end
    while task.wait(60)do if AR then pcall(doRift)end end
end)
